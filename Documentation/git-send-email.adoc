...
--sendmail-cmd=<command>::
	...
	In addition to that, any developer can also add the following
	`git send-email` specific outputs to the stdout of the application:

	----
	git_result=some_result # Prints the response code. Eg: Result: 202
	git_message_id=<some_message_id> # Prints the Message-ID of the sent email if changed.
	----
